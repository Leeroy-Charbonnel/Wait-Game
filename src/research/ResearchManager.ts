...
export class ResearchManager {
  ...
  public startResearch(id: ResearchID): boolean {
    ...
    if (this.isResearchCompleted(id)) {
      return false;
    }
  ...
  public update(currentTime: number): void {
    ...
    if (elapsed >= this.activeResearch.duration) {
      ...
      if (research) {
        ...
        // Grant rewards
        this.grantRewards(research.rewards);
  ...
  private grantRewards(rewards: ResearchReward[]): void {
    rewards.forEach(reward => {
      switch (reward.type) {
        case 'resource':
          this.game.resourceManager.addResource(reward.id as any, reward.amount || 0);
          break;
